...
<div class="bg-white rounded-lg shadow-md p-6 mb-6">
    <div class="flex items-center justify-between mb-4">
        <h3 class="text-lg font-semibold text-gray-800">
            <i class="fas fa-chart-pie text-blue-500 mr-2"></i>{{ __('messages.profile.completeness') }}
        </h3>
    ...
    @if(count($missing) > 0)
        <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            <h4 class="text-sm font-medium text-yellow-800 mb-2">
                <i class="fas fa-exclamation-triangle mr-1"></i>{{ __('messages.profile.incomplete') }}
            </h4>
            ...
            <div class="mt-3">
                <a href="{{ route('profile.edit') }}" 
                   class="inline-flex items-center px-3 py-2 bg-yellow-600 text-white text-sm font-medium rounded-md hover:bg-yellow-700 transition duration-200">
                    ...
                    {{ __('messages.profile.complete_profile') }}
                </a>
            </div>
        </div>
    @else
        <div class="bg-green-50 border border-green-200 rounded-lg p-4">
            <div class="flex items-center">
                ...
                <span class="text-green-800 font-medium">{{ __('messages.profile.complete') }}</span>
            </div>
        </div>
    @endif
    ...
    @if($completeness < 75)
        <div class="mt-4 bg-blue-50 border border-blue-200 rounded-lg p-4">
            <h4 class="text-sm font-medium text-blue-800 mb-2">
                <i class="fas fa-lightbulb mr-1"></i>{{ __('messages.profile.tips_title') }}
            </h4>
            <ul class="text-sm text-blue-700 space-y-1">
                <li>{{ __('messages.profile.tip_bio') }}</li>
                <li>{{ __('messages.profile.tip_interests') }}</li>
                <li>{{ __('messages.profile.tip_photos') }}</li>
                <li>{{ __('messages.profile.tip_psychological') }}</li>
            </ul>
        </div>
    @endif
</div>
